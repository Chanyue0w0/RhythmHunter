...
- The project default pipeline uses `Assets/Settings/Renderer2D.asset`. For gameplay scenes, choose the package Prefabs whose names start with `VFX_2D_`; they reference the package's `Shared/2D Materials` and shaders with a `Universal2D` pass.
- The vendor Demo Scene references the standard `VFX_` Prefabs, whose shaders use `UniversalForwardOnly`, while its camera inherits this project's default Renderer2D (`Renderer Index = -1`). Those effects therefore do not render correctly in the Demo under the current project settings.
- To preview the vendor Demo without changing gameplay rendering, add a Universal/Forward Renderer as a secondary renderer on `UniversalRP.asset` and select that renderer only on each Demo camera. Do not replace the project-wide Renderer2D default. The alternative is to build a separate 2D preview using the `VFX_2D_` Prefabs.

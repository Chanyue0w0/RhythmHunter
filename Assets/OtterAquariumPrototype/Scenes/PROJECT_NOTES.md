...
- Demo1 now has an optional cinematic wrapper on `Demo1CombatController > Otter Goblin Demo1 Cinematic Director`. Clear `Enable Cinematics` before Play to start FMOD and test combat immediately. When enabled, music/chart input is gated until the complete opening finishes: the camera follows the otter swimming in, pans to the Goblin, shows the two-second/any-key dialogue, then follows a thrown axe in slow motion until the otter catches and cracks it. The cinematic catch deliberately plays the complete Cracking sequence at 2.5 times the normal duration so frames 2–4 and the shatter impact remain readable; normal rhythm catches retain their fast timing. The battle camera and normal time scale are restored before FMOD starts. `Additional Opening Timeline` and `Ending Timeline` are optional empty slots for future authored sequences; a populated ending Timeline is played after `BattleWon`.
...
- The otter now uses only the current non-`old` art: it loops Swimming before the first phrase, rolls for two beats, settles into the back-floating Idle pose for two beats, then bobs and rocks from the live song tick. Perfect/Good catches jump to Cracking frame 2, carry the caught projectile from the hands to the belly through frames 3–4, and expose a `CrackImpact` event for additional authored VFX. A dedicated Food animation remains reserved for later.
- `cracking_no stone_1` remains an unused preparation pose by design; catch reactions begin on `cracking_no stone_2`. The cinematic adds a separate 0.5-beat hold to that catch pose before advancing through frames 3–4, while combat keeps its original short response. During normal Intro/Rest idle phases, the four Rubbing frames occasionally play once at a random 6–12 second interval; a warning or catch interrupts the variation immediately.
- During the first four music beats, the Swimming animation moves in from 5.5 world units beyond the otter's final position on the right. Rolling begins only after the visual reaches its final position. `OtterCombatAnimator` updates after the rhythm Runner, and its Idle float reaches the top of the bob exactly on each authored beat.
- Every caught projectile Sprite, including Prefabs currently marked as Food, is divided into a 3-by-3 set of runtime fragments at the belly impact. Fragments preserve the source texture, color, sorting, rotation, and scale, then scatter with gravity and fade over roughly 1.25 music beats. Food temporarily uses the same cracking animation until its dedicated eating animation is available.
- Special projectile screen interference is authored directly on each Prefab through `Screen Interference` and `Interference Duration Beats`, so its real-time duration follows the current FMOD tempo. `dying_bottle` produces a layered orange ink spill over the right half of the screen for four beats; `save_icon` displays a dimmed `SAVE... / LOADING` panel, its own icon, animated dots, and a progress bar for three beats. The effect triggers once when the projectile reaches its target or is caught early, regardless of Perfect/Good/Miss, and repeated matching effects refresh rather than stacking to unreadable opacity.
- The full-screen red warning pulse is disabled through `Show Warning Danger Flash`; miss/hurt feedback remains enabled. The setting can be restored later without changing warning timing or audio.
...
- Triple axes are pre-scheduled against their individual catch targets. Each axe becomes visible half a beat before its own `X'`: the first throw now begins on the rest after `X X X`, instead of appearing during the third warning, while all three retain equal visual reaction time and the `X' _ X' _ X'` input spacing remains unchanged.
...
- Pressing defend with no attack inside the Good window triggers `BeatMiss` and adds one Extra. `亂按硬直（拍）` is configurable per LevelData from zero upward: zero keeps the audio/stat feedback but applies no input lock, while positive values lock defend input for that many music beats. The stray press itself deals no damage, but an incoming projectile can still become a normal damaging Miss.

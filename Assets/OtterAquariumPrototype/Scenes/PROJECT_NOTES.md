...
- Each Demo1 phrase now exposes one projectile Prefab slot for every player catch `X'` in the Rhythm Level Editor. Leave a slot empty to use `GoblinFlyingAxe`, or drag in any Prefab that contains `RhythmTimelineProjectile`; triple and combined phrases keep their catch-order mapping.
